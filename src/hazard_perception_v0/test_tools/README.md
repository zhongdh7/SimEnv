...
`/hazard_perception_v0/hazards_3d` 观察正式感知节点；它不会发布真值话题，也没有被
...
catkin_make --pkg hazard_perception_v0 -j2
...
source /home/lin/SimEnv/devel/setup.bash
source devel/setup.bash
roslaunch src/hazard_perception_v0/test_tools/hazard_validation.launch
```

如果正式流水线已经运行，避免节点重名：

```bash
roslaunch src/hazard_perception_v0/test_tools/hazard_validation.launch start_pipeline:=false
```
...
rosbag record -O /home/lin/simenv_algorithm_ws/results/hazard_validation/bags/rgbd_validation.bag \
  ...
  /hazard_perception_v0/detections \
  /hazard_perception_v0/hazards_3d \
  /hazard_perception_v0/geometry_diagnostics \
  /hazard_perception_v0/pipeline_debug_image
```
...
source /opt/ros/noetic/setup.bash
source devel/setup.bash
roslaunch hazard_perception_v0 hazard_pipeline.launch use_livox:=false
```

先启动流水线，再在播放终端按空格。为了避免与 bag 内已录制输出冲突，若要重新计算，
可用 `rosbag filter` 去掉 `/hazard_perception_v0/*` 输出，或录制时只录输入与 TF。独立自动
...
source devel/setup.bash
python3 -m unittest \
  src/hazard_perception_v0/test_tools/test_validation_metrics.py -v
roslaunch --nodes src/hazard_perception_v0/test_tools/hazard_validation.launch
```
