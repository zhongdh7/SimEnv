...
int main(int argc, char **argv)
{
  ...
  // 数据文件路径，通过环境变量 DATA_ROOT 指定，默认为当前目录下的 data.txt
  const char* data_root_env = std::getenv("DATA_ROOT");
  string data_root = (data_root_env != nullptr) ? string(data_root_env) : ".";
  string groundtruth_path = data_root + "/data.txt";
  std::fstream gt_file;
