  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::multi_map_server::VerticalOccupancyGridList_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.upper.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<int32_t>::stream(s, true ? std::string() : indent + "    ", v.upper[i]);
    }
    if (v.upper.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.lower.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<int32_t>::stream(s, true ? std::string() : indent + "    ", v.lower[i]);
    }
    if (v.lower.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.mass.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<int32_t>::stream(s, true ? std::string() : indent + "    ", v.mass[i]);
    }
    if (v.mass.empty() || true)
      s << "]";
  }
