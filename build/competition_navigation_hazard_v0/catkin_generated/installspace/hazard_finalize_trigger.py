...
The C++ navigation stack and hazard_perception_v0 run independently (shallow
...
it calls the /hazard_perception_v0/finalize service so the detected hazard
coordinates are written to the results directory automatically — no manual
`rosservice call` needed.

This is the v0-hazard variant: it finalizes the hazard_perception_v0 pipeline.
...
_FINALIZE_SERVICE = "/hazard_perception_v0/finalize"
...
class FinalizeTrigger:
    def __init__(self):
        ...
        rospy.loginfo(
            "hazard_finalize_trigger(v0): watching nav status for completion"
        )
    ...
    def _on_status(self, msg):
        ...
        try:
            ...
            resp = self._service()
            rospy.loginfo(
                "hazard_finalize_trigger(v0): finalized (success=%s, msg=%s)",
                resp.success, resp.message,
            )
        except rospy.ServiceException as exc:
            rospy.logwarn(
                "hazard_finalize_trigger(v0): finalize call failed: %s", exc
            )
...
def main():
    rospy.init_node("hazard_finalize_trigger_v0")
    FinalizeTrigger()
    rospy.spin()
