...
from hazard_perception.msg import (
    Hazard3D,
    Hazard3DArray,
    HazardDetection2D,
...
class HazardPipelineNode:
    def __init__(self):
        ...
        self.result_manager = ResultManager(
            ...
            floor_height_m=getattr(self, "floor_height_m", 2.6),
            floor_count=getattr(self, "floor_count", 3),
            sphere_radius_m=getattr(self, "sphere_radius", 0.15),
        )
    ...
    def _load_parameters(self):
        ...
        self.hazards_topic = get("~hazards_topic", "/hazard_perception/hazards_3d")
        self.detections_topic = get("~detections_topic", "/hazard_perception/detections")
        self.geometry_diagnostics_topic = get(
            "~geometry_diagnostics_topic", "/hazard_perception/geometry_diagnostics"
        )
        self.markers_topic = get("~markers_topic", "/hazard_perception/markers")
        self.pipeline_mask_topic = get(
            "~pipeline_mask_topic", "/hazard_perception/pipeline_red_mask"
        )
        self.pipeline_debug_topic = get(
            "~pipeline_debug_topic", "/hazard_perception/pipeline_debug_image"
        )
        self.finalize_service_name = get("~finalize_service", "/hazard_perception/finalize")
        ...
        self.floor_height_m = float(get("~floor_height_m", 2.6))
        self.floor_count = max(1, int(get("~floor_count", 3)))
        ...
        self.edge_confirmation_count = int(get("~edge_confirmation_count", 2))
