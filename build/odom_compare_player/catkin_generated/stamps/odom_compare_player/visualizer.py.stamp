...
def main():
    ...
    gt_marker = _line_strip(frame, "gt", (0.0, 1.0, 0.0, 1.0), 0.10, gt_pts)
    lio_marker = _line_strip(frame, "lio", (1.0, 0.0, 0.0, 1.0), 0.10, lio_pts)
    ...
    dev_marker = _line_list(frame, "deviation", (0.3, 0.3, 1.0, 0.8), 0.03, dev_pts)

    # A latched publisher keeps only the LAST message, so a subscriber that
    # connects after startup would only ever receive "deviation" and miss the
    # gt / lio lines.  Re-publish all three periodically so RViz accumulates
    # every marker regardless of when it subscribes.
    def republish(_event):
        now = rospy.Time.now()
        for m in (gt_marker, lio_marker, dev_marker):
            m.header.stamp = now
            pub.publish(m)

    republish(None)
    rospy.Timer(rospy.Duration(0.5), republish)
    ...
    rospy.loginfo("spinning to re-publish markers (Ctrl-C to quit)")
    rospy.spin()
