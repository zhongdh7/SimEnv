...
class CollisionSafety(object):
    def __init__(self):
        ...
        # Minimum lateral (perpendicular-to-travel) run of continuous OCCUPIED
        # cells that marks an obstacle as a genuine wall (or door frame) rather
        # than a small ghost cluster left by 3-D flattening / drift.  A real
        # wall extends far laterally; a ghost cluster is only a few cells wide
        # (measured corridor ghosts: 2-4 cells = 0.4-0.8 m).  5 cells = 1.0 m.
        self.wall_run_threshold = max(
            1, int(rospy.get_param("~wall_run_threshold", 5))
        )
    ...
    def _obstacle_is_isolated(self, x, y, cos_yaw, sin_yaw, d):
        """True when the blocked cell belongs to a small phantom cluster.

        A ghost cell/cluster left by 3-D flattening or drift is only a few
        cells wide (measured corridor ghosts: 2-4 cells = 0.4-0.8 m).  A real
        wall -- including a door frame -- extends laterally for far longer.
        We therefore measure the continuous OCCUPIED run perpendicular to
        travel on each side and treat the obstacle as a phantom only when BOTH
        sides stay below ``wall_run_threshold``.  This lets small ghost
        clusters pass (so the robot is not frozen in a corridor with ghost
        debris) while still releasing nothing that looks like a genuine wall,
        whose far side may already be scanned FREE room interior (the doorway
        case)."""
        ox = x + d * cos_yaw
        oy = y + d * sin_yaw
        for side in (-1, 1):
            run = 0
            for lat_cells in range(1, 1 + self.wall_run_threshold + 2):
                ...
                if self._occ(px, py) == OCCUPIED:
                    ...
                    break
            if run >= self.wall_run_threshold:
                return False
        return True
