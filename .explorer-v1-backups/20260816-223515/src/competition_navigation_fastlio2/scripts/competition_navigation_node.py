    ...
    def scan_callback(self, msg):
        ...
        with self.lock:
            ...
            for point in transformed:
                ...
                for cell_x, cell_y in line[:-1]:
                    if self._inside(cell_x, cell_y):
                        if belief[cell_y, cell_x] == OCCUPIED:
                            # A free ray now crosses an occupied cell: it may be
                            # a ghost obstacle from 3-D flattening.  Accumulate
                            # free passes and clear it only once the free-pass
                            # fraction exceeds obstacle_clear_percent (after
                            # obstacle_clear_min_obs observations).  A real wall
                            # keeps being hit, so its fraction stays low and it
                            # survives.
                            free = self.obs_free[cell_y, cell_x] + 1
                            self.obs_free[cell_y, cell_x] = free
                            total = free + self.obs_hits[cell_y, cell_x]
                            if (total >= self.obstacle_clear_min_obs and
                                    free * 100.0 / total >
                                    self.obstacle_clear_percent):
                                belief[cell_y, cell_x] = FREE
                                self.obs_free[cell_y, cell_x] = 0
                                self.obs_hits[cell_y, cell_x] = 0
                        else:
                ...
                if self._inside(ex, ey):
                    ...
                    if belief[ey, ex] == OCCUPIED:
                        self.obs_hits[ey, ex] += 1
                    else:
                        self.obs_hits[ey, ex] = 1
                        self.obs_free[ey, ex] = 0
            ...
            if self._inside(robot_cell[0], robot_cell[1]):
                ...
                self.obs_free[robot_cell[1], robot_cell[0]] = 0
                self.obs_hits[robot_cell[1], robot_cell[0]] = 0
