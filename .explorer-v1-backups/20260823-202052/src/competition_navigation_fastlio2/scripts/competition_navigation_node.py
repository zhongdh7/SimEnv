...
class CompetitionNavigation(object):
    ...
    def _plan_graph_room_task(self, belief, pose, floor_index):
        ...
        if (floor_index >= len(self.graph_phase) or
            ...
            # A late scan may confirm a doorway AFTER the DFS walk finished
            # (graph_phase = frontier_fallback).  Such a room would never be
            # processed, so _graph_floor_ready stays False and the floor can
            # never complete (measured: floor-2 L:31.15 phantom door stuck the
            # floor in survey_coverage_incomplete forever).  Resume DFS when
            # any discovered room is still neither completed nor blocked.
            if (floor_index < len(self.graph_phase) and
                ...
                if pending and self._prepare_graph_dfs(floor_index):
                    self.graph_phase[floor_index] = "dfs"
                    self.status = "dfs_resumed_late_rooms=%d" % len(pending)
                    return True
            return False
        graph = self.floor_graphs[floor_index]
