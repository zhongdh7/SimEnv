...
- 未融合 FAST-LIO/IMU(`use_odometry=false`);如需更高频输出可后续接入 `use_odometry=true`
  与帧重映射。
- 已接入导航:`launch/carto_nav.launch`(集成 FAST-LIO2 + Cartographer + `carto_relay` +
  导航)让 `competition_navigation_fastlio2` 用回环矫正后的位姿定位。`carto_relay` 把
  Cartographer 的 `carto_map→base` 重锚回建筑对齐的 `odom` 系(导航启发式非坐标系无关,
  必须用 odom,否则地图整体转 ~90° 会往左跑),再发 `/carto_odom` + `/scan_carto`;导航以
  `map_frame=odom` 消费。用 `scripts/start_carto_navigation.sh` 启动。`slam_nav.launch`
  仍用原流程。
