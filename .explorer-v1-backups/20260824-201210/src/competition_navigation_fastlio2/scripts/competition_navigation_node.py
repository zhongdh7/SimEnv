    ...
    def control_timer(self, _event):
        ...
        if mode in ("stairs_up", "stairs_down"):
            ...
            if cruise_command is not None:
                ...
                # Flight segment not yet cruise-aligned: hold FORWARD motion and
                # rotate + strafe in place (angular.z and linear.y below stay
                # active) until heading/lateral error fall inside the cruise
                # window.  Driving forward while misaligned is what slips the
                # robot off the stair edge -- the dominant cause of both up- and
                # down-stair falls.  Lateral strafe is deliberately NOT zeroed
                # so the robot can recentre itself against the stair axis.
