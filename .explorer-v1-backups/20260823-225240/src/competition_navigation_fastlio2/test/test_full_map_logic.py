...
from competition_navigation_node import (
    CompetitionNavigation,
    DEFAULT_SCAN_MIN_RANGE,
    DEFAULT_SCAN_SELF_FILTER_FRONT,
    DEFAULT_SCAN_SELF_FILTER_REAR,
    FREE,
    OCCUPIED,
    UNKNOWN,
)
...
def _navigation_stub(size=60):
    ...
    navigation.room_door_ys_by_floor = [[], [], []]
    navigation.room_entry_observations = [dict(), dict(), dict()]
    navigation.expected_room_count = [0, 0, 0]
    navigation.active_room_node = None
    navigation.room_frontier_min_cluster_cells = 3
    navigation.room_candidates_per_cluster = 2
    navigation.room_candidate_limit = 10
    navigation.room_information_range = 6.0
    navigation.room_information_angle_step_deg = 10.0
    navigation.room_path_cost_weight = 0.45
    navigation.room_return_cost_weight = 0.20
    navigation.room_obstacle_cost_weight = 0.80
    navigation.room_target_blacklist_radius = 0.80
    navigation.room_entry_confirmation_cycles = 2
    navigation.room_entry_allow_unknown_wall = True
    navigation.active_room_target_cell = None
    navigation.active_room_frontier_blacklist = set()
    navigation.last_room_plan = None
    room_id, _ = navigation.floor_graphs[0].add_room_entry(
        0, -1.1, 14.865, corridor_s=14.865
    )
    second_room_id, _ = navigation.floor_graphs[0].add_room_entry(
        0, -1.1, 28.895, corridor_s=28.895
    )
    navigation.floor_graphs[0].mark_node(room_id, completed=True)
    navigation.floor_graphs[0].mark_node(second_room_id, completed=True)
    navigation._refresh_room_door_ys(0)
    return navigation


class FullMapLogicTest(unittest.TestCase):
    def test_global_scan_default_excludes_near_self_returns(self):
        self.assertAlmostEqual(DEFAULT_SCAN_MIN_RANGE, 2.0)

    def test_self_filter_rejects_robot_body_but_keeps_distant_obstacle(self):
        navigation = _navigation_stub()
        navigation.scan_self_filter_front = DEFAULT_SCAN_SELF_FILTER_FRONT
        navigation.scan_self_filter_rear = DEFAULT_SCAN_SELF_FILTER_REAR
        navigation.scan_self_filter_left = 0.55
        navigation.scan_self_filter_right = 0.55

        self.assertTrue(
            navigation._is_self_return((0.5, 0.0), (0.0, 0.0, 0.3, 0.0))
        )
        self.assertTrue(
            navigation._is_self_return((-0.4, 0.1), (0.0, 0.0, 0.3, 0.0))
        )
        self.assertFalse(
            navigation._is_self_return((1.2, 0.0), (0.0, 0.0, 0.3, 0.0))
        )
        self.assertFalse(
            navigation._is_self_return((0.0, 0.8), (0.0, 0.0, 0.3, 0.0))
        )

    def test_room_entry_requires_persistent_observation(self):
        navigation = _navigation_stub()
        candidate = {
            "side": "left",
            "x": -1.7,
        ...
        self.assertEqual(
            navigation._confirmed_room_entry_candidates([candidate], 0), []
        )
        confirmed = navigation._confirmed_room_entry_candidates([candidate], 0)
        self.assertEqual(len(confirmed), 1)
        self.assertEqual(confirmed[0]["side"], "left")

    def test_room_frontier_clusters_use_eight_connectivity(self):
        frontier = np.zeros((12, 12), dtype=bool)
        frontier[2, 2:5] = True
        frontier[8, 8] = True
        frontier[9, 9] = True

        clusters = CompetitionNavigation._frontier_clusters(frontier)

        self.assertEqual(sorted(len(cluster) for cluster in clusters), [2, 3])

    def test_room_information_gain_is_occlusion_aware(self):
        navigation = _navigation_stub(size=30)
        navigation.cell_size = 1.0
        navigation.room_information_range = 8.0
        navigation.room_information_angle_step_deg = 5.0
        room_mask = np.ones((30, 30), dtype=bool)
        belief = np.full((30, 30), UNKNOWN, dtype=np.uint8)
        belief[15, 10] = FREE

        open_gain = navigation._room_information_gain(
            belief, room_mask, (10, 15)
        )
        belief[:, 14] = OCCUPIED
        blocked_gain = navigation._room_information_gain(
            belief, room_mask, (10, 15)
        )

        self.assertLess(blocked_gain, open_gain)

    def test_room_frontier_plan_records_gain_and_return_cost(self):
        navigation = _navigation_stub(size=30)
        navigation.cell_size = 1.0
        navigation.map_origin_x = 0.0
        navigation.map_origin_y = 0.0
        navigation.room_frontier_min_cluster_cells = 2
        navigation._room_bounds = lambda _node: {
            "x_min": 0.0, "x_max": 29.0,
            "y_min": 0.0, "y_max": 29.0,
        }
        navigation._room_mask = lambda _bounds: np.ones((30, 30), dtype=bool)
        navigation._known_free_mask = lambda belief: belief == FREE
        belief = np.full((30, 30), UNKNOWN, dtype=np.uint8)
        belief[8:22, 2:14] = FREE
        node = SimpleNamespace(position=(2.0, 15.0))

        path = navigation._room_frontier_path(
            belief, (3.0, 15.0, 0.0, 0.0), node
        )

        self.assertTrue(path)
        self.assertIsNotNone(navigation.active_room_target_cell)
        self.assertGreater(navigation.last_room_plan["gain_cells"], 0)
        self.assertGreaterEqual(navigation.last_room_plan["return_m"], 0.0)
    ...
    def test_stair_run_tracking_uses_moving_centerline_lookahead(self):
        ...
        self.assertEqual(tracking, (-4.018, 2.30))
        self.assertEqual(
    ...
    def test_stair_cruise_keeps_speed_on_aligned_flight(self):
        self.assertAlmostEqual(
            CompetitionNavigation._stair_cruise_command(
                "stairs_up", 1, 0.12, 0.02, 0.10, 2.0, 0.40, 0.45
            ),
            0.40,
        )

    def test_stair_cruise_brakes_near_landing_or_bad_alignment(self):
        cruise = CompetitionNavigation._stair_cruise_command
        self.assertIsNone(
            cruise("stairs_up", 1, 0.12, 0.02, 0.10, 0.30, 0.40, 0.45)
        )
        self.assertIsNone(
            cruise("stairs_up", 1, 0.12, 0.22, 0.10, 2.0, 0.40, 0.45)
        )
        self.assertIsNone(
            cruise("stairs_up", 1, 0.12, 0.02, 0.70, 2.0, 0.40, 0.45)
        )
    ...
    def test_stair_yaw_limit_overcomes_platform_turning_deadband(self):
        ...
        self.assertAlmostEqual(
            navigation._yaw_rate_limit("stairs_up", 1.2), 1.0
        )
        self.assertAlmostEqual(
        ...
        self.assertAlmostEqual(
            navigation._yaw_rate_limit("stairs_up", 1.2, 0.8), 0.8
        )
        self.assertAlmostEqual(
            navigation._yaw_rate_limit("stairs_up", 0.7, 1.2), 0.7
        )

    def test_stair_speed_cap_splits_route_segments(self):
        cap = CompetitionNavigation._stair_speed_cap

        for index in (0, 4):
            self.assertAlmostEqual(
                cap("stairs_up", index, 0.60, 0.40, 1.00), 1.00
            )
            self.assertAlmostEqual(
                cap(
                    "stairs_up",
                    ...
                    0.40,
                    1.00,
                    flat_use_max_linear=False,
                ),
                0.60,
            )
        for index in (1, 3):
            self.assertAlmostEqual(
                cap("stairs_down", index, 0.60, 0.40, 1.00), 0.60
            )
        self.assertAlmostEqual(
            cap("stairs_up", 2, 0.60, 0.40, 1.00), 0.40
        )
        self.assertAlmostEqual(
            cap("explore", 2, 0.60, 0.40, 1.00), 1.00
        )
        self.assertAlmostEqual(
            cap("stairs_up", 5, 0.60, 0.40, 1.00), 1.00
        )
    ...
    def _paint(self, belief, navigation, x, y0, y1, value):
        """Paint belief cells along a vertical line segment (world coords)."""
        cx = int(round(x / navigation.cell_size))
        cy0 = int(round(y0 / navigation.cell_size))
        cy1 = int(round(y1 / navigation.cell_size))
        for cy in range(cy0, cy1 + 1):
            if 0 <= cy < navigation.map_cells and 0 <= cx < navigation.map_cells:
                belief[cy, cx] = value

    def _phantom_filter_navigation(self):
        navigation = _navigation_stub(size=60)
        navigation.room_entry_min_width_cells = 3
        navigation.room_entry_wall_support_cells = 3
        navigation.room_entry_wall_support_min_occupied = 2
        navigation.room_entry_max_width_cells = 7
        return navigation

    def test_room_entry_accepts_real_door_with_solid_wall_flanks(self):
        navigation = self._phantom_filter_navigation()
        belief = np.full(
            (navigation.map_cells, navigation.map_cells), UNKNOWN, dtype=np.uint8
        )
        wall_x = navigation.corridor_bounds["x_min"]  # -1.1
        # Corridor interior and room interior are free.
        self._paint(belief, navigation, -0.65, 8.65, 20.0, FREE)
        self._paint(belief, navigation, -1.55, 8.65, 20.0, FREE)
        # Solid wall above and below the opening (O-F-O profile).
        self._paint(belief, navigation, wall_x, 8.65, 14.25, OCCUPIED)
        self._paint(belief, navigation, wall_x, 15.85, 20.0, OCCUPIED)
        # Door opening: 3 free wall cells centred near y=14.65.
        self._paint(belief, navigation, wall_x, 14.65, 15.45, FREE)

        candidates = navigation._room_entry_candidates(belief)
        left = [c for c in candidates if c["side"] == "left"]
        self.assertEqual(len(left), 1, "real door with solid flanks must pass")
        self.assertAlmostEqual(left[0]["y"], 15.05, delta=0.41)

    def test_room_entry_rejects_wide_broken_wall_opening(self):
        navigation = self._phantom_filter_navigation()
        belief = np.full(
            (navigation.map_cells, navigation.map_cells), UNKNOWN, dtype=np.uint8
        )
        wall_x = navigation.corridor_bounds["x_min"]
        self._paint(belief, navigation, -0.65, 8.65, 20.0, FREE)
        self._paint(belief, navigation, -1.55, 8.65, 20.0, FREE)
        # Whole wall segment free -> a single very wide "opening" (> 7 cells).
        self._paint(belief, navigation, wall_x, 8.65, 20.0, FREE)

        candidates = navigation._room_entry_candidates(belief)
        self.assertEqual(
            candidates, [], "wide broken-wall opening must be rejected"
        )

    def test_room_entry_rejects_narrow_opening_without_wall_support(self):
        navigation = self._phantom_filter_navigation()
        belief = np.full(
            (navigation.map_cells, navigation.map_cells), UNKNOWN, dtype=np.uint8
        )
        ...
        self._paint(belief, navigation, -0.65, 8.65, 20.0, FREE)
        self._paint(belief, navigation, -1.55, 8.65, 20.0, FREE)
        # Broken wall: alternating O/F along the wall line, with a 3-cell free
        # gap whose 3-cell windows on both sides contain fewer than 2 occupied.
        self._paint(belief, navigation, wall_x, 8.65, 13.45, OCCUPIED)
        self._paint(belief, navigation, wall_x, 13.85, 13.85, FREE)
        self._paint(belief, navigation, wall_x, 14.25, 14.25, OCCUPIED)
        self._paint(belief, navigation, wall_x, 14.65, 15.45, FREE)
        self._paint(belief, navigation, wall_x, 15.85, 15.85, OCCUPIED)
        self._paint(belief, navigation, wall_x, 16.25, 16.25, FREE)
        self._paint(belief, navigation, wall_x, 16.65, 20.0, OCCUPIED)

        candidates = navigation._room_entry_candidates(belief)
        self.assertEqual(
            candidates, [], "opening with no solid wall side must be rejected"
        )

    def test_room_entry_accepts_door_with_one_solid_side(self):
        navigation = self._phantom_filter_navigation()
        belief = np.full(
            (navigation.map_cells, navigation.map_cells), UNKNOWN, dtype=np.uint8
        )
        ...
        self._paint(belief, navigation, -0.65, 8.65, 20.0, FREE)
        self._paint(belief, navigation, -1.55, 8.65, 20.0, FREE)
        # Below the gap the wall is broken (all FREE), above it solid.
        self._paint(belief, navigation, wall_x, 8.65, 13.45, FREE)
        self._paint(belief, navigation, wall_x, 14.65, 15.45, FREE)
        self._paint(belief, navigation, wall_x, 15.85, 20.0, OCCUPIED)
        ...
        left = [c for c in candidates if c["side"] == "left"]
        self.assertEqual(
            len(left), 1, "door with at least one solid wall side must pass"
        )
