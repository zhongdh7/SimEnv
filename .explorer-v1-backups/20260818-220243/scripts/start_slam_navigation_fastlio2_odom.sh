...
(competition_navigation_fastlio2) uses FAST-LIO2's /Odometry re-anchored into
the building-aligned odom frame (published as /Odometry_fastlio by
scan_tf_relay) as its odometry source instead of Gazebo ground truth.  The
exploration logic is the same competition_navigation_node.py, re-wired via
scan_tf_relay.
