...
float readStandDuration()
{
    ...
    char *end = nullptr;
    errno = 0;
    const float duration = std::strtof(envValue, &end);
    if(errno != 0 || end == envValue || *end != '\0' ||
       !std::isfinite(duration) || duration < 0.5f || duration > 8.0f){
...
float readStandSettleDuration()
{
    ...
    const float duration = std::strtof(envValue, &end);
    if(errno != 0 || end == envValue || *end != '\0' ||
       !std::isfinite(duration) || duration < 0.0f || duration > 3.0f){
