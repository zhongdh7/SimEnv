...
class CompetitionNavigation(object):
    def __init__(self):
        ...
        # How many consecutive free-ray crossings an occupied cell needs before
        # it is cleared back to free (see scan_callback).  Higher = more robust
        # to pose jitter but slower to drop stale/ghost obstacles.
        self.obstacle_clear_hits = max(
            1, int(rospy.get_param("~obstacle_clear_hits", 3))
        )
        ...
        # Consecutive free-ray crossings through an occupied cell (see
        # scan_callback).  Lets stale/ghost obstacles be cleared without letting
        # a single noisy ray erode a real wall.
        self.clear_confirm = [
            np.zeros((cells, cells), dtype=np.int16)
            for _ in range(self.floor_count)
        ]
    ...
    def scan_callback(self, msg):
        ...
        with self.lock:
            ...
            clear_confirm = self.clear_confirm[floor_index]
            new_known_cells = 0
                ...
                for cell_x, cell_y in line[:-1]:
                    if self._inside(cell_x, cell_y):
                        if belief[cell_y, cell_x] == OCCUPIED:
                            # A free ray now crosses a cell previously marked
                            # occupied: it may be a stale/ghost obstacle.  Clear
                            # it only after enough consecutive free crossings so
                            # a real wall (which keeps being re-hit) survives.
                            hits = clear_confirm[cell_y, cell_x] + 1
                            if hits >= self.obstacle_clear_hits:
                                belief[cell_y, cell_x] = FREE
                                clear_confirm[cell_y, cell_x] = 0
                            else:
                                clear_confirm[cell_y, cell_x] = hits
                        else:
            ...
            if self._inside(robot_cell[0], robot_cell[1]):
                ...
                clear_confirm[robot_cell[1], robot_cell[0]] = 0
