...
class CompetitionNavigation(object):
    ...
    def control_timer(self, _event):
        ...
        if mode in ("stairs_up", "stairs_down"):
            ...
            # Continuous forward motion on the flight segments (index 1, 3): do
            # NOT zero forward when slightly misaligned.  Stop-and-go on the
            # stairs loses forward momentum and the quadruped slips off a tread
            # edge.  When the cruise window is not met, the normal proportional
            # controller (computed above) keeps driving forward while the
            # angular/lateral correction steers it back onto the stair axis.
