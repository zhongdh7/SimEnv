...
- 未融合 FAST-LIO/IMU(`use_odometry=false`);如需更高频输出可后续接入 `use_odometry=true`
  与帧重映射。
- 已接入导航:`launch/carto_nav.launch`(集成 FAST-LIO2 + Cartographer + 桥 + 导航)让
  `competition_navigation_fastlio2` 在 `map_frame=carto_map` 下定位;用
  `scripts/start_carto_navigation.sh` 启动。`slam_nav.launch` 仍用 `map_frame=odom`(原流程不变)。
