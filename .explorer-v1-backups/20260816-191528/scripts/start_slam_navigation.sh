...
usage() {
  cat <<'EOF'
Start FAST-LIO SLAM + competition navigation + PCD saving.

Sourcing order matters: SimEnv and ws_livox are two independent catkin
workspaces whose setup.bash files overwrite each other's ROS_PACKAGE_PATH.
This script sources both and then merges their src paths explicitly.

Usage:
  start_slam_navigation.sh [--simenv PATH] [--wslivox PATH] [-- roslaunch-arg:=value ...]

Options:
  --simenv PATH     SimEnv workspace (default: $HOME/SimEnv)
  --wslivox PATH    ws_livox workspace containing FAST-LIO (default: $HOME/ws_livox)
...
rospack find fastlio_slam_merge >/dev/null 2>&1 || \
  die "ROS package fastlio_slam_merge is unavailable; install and build explorer-v1 first"
rospack find fast_lio >/dev/null 2>&1 || \
  die "ROS package fast_lio is unavailable; build ws_livox first"

echo "Starting FAST-LIO SLAM + competition navigation"
echo "  SimEnv: $SIMENV"
echo "  ws_livox: $WSLIVOX"
exec roslaunch fastlio_slam_merge slam_nav.launch "${EXTRA_ARGS[@]}"
